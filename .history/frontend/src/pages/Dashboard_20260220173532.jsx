...
export default function Dashboard() {
  ...
  // sirf task owner hi delete ya update kar sakta hai
  // admin delete kar sakta hai (moderation ke liye) but update nahi
  const isOwner = (task) => {
    return task.user === userId || task.userId === userId || task.createdBy === userId;
  };

  const canUpdate = (task) => isOwner(task);             // sirf owner update kar sakta hai
  const canDelete = (task) => role === "admin" || isOwner(task); // admin ya owner dono delete kar sakte hain
